...
CHECK_INTERVAL_SECONDS = int(os.environ.get("CHECK_INTERVAL_SECONDS", "1500"))
...
# 已移除 Scamalytics / ProxyCheck 第三方质量校验。
# 保留这两个开关变量，是为了兼容 IPAPI_REJECT_CONFIG 里的引用，避免 NameError。
# 后续不再读取 SCAMALYTICS_API_URL_TEMPLATE / PROXYCHECK_API_URL_TEMPLATE，
# 也不会再请求 Scamalytics / ProxyCheck 接口。
QUALITY_CHECK_SCAMALYTICS_ENABLED = False
QUALITY_CHECK_PROXYCHECK_ENABLED = False

QUALITY_SCAMALYTICS_STRICT = False
QUALITY_PROXYCHECK_STRICT = False
QUALITY_REJECT_PROXYCHECK_PROXY = False
QUALITY_MAX_SCAMALYTICS_SCORE = 0
...
def has_confirmed_active_vpn_connection() -> bool:
    ...
    state = read_json(STATE_FILE, {})
    return (
            state.get("connection_stage") == "connected"
            and bool(resolve_active_openvpn_node_id())
...
def connect_node(node_id: str) -> str:
    ...
    try:
        ...
        set_state(
            active_openvpn_node_id=node_id,
            is_connecting=True,
            connection_stage="quality_checking",
            quality_checking_node_id=node_id,
            last_check_message="OpenVPN 已建立，正在检测出口 IP 质量与 7928 代理下载速度...",
...
def probe_quality_via_interface(interface: str, include_speed: bool = True) -> tuple[bool, str, dict[str, Any]]:
    ...
    quality_info.update({
        "scamalytics_checked": False,
        "has_scamalytics_score": False,
        "scamalytics_score": 0,
        "scamalytics_risk": "",
        "scamalytics_url": "",
        "scamalytics_isp": "",
        ...
        "proxycheck_checked": False,
        "has_proxycheck_proxy": False,
        "proxycheck_proxy": "",
        "proxycheck_is_proxy": False,
        "proxycheck_type": "",
        "proxycheck_ip": str(quality_info.get("ip") or ""),
        "proxycheck_error": "",
    })
...
def update_node_quality(node_id: str, quality_info: dict[str, Any], passed: bool, reason: str) -> None:
    with lock:
        ...
        if quality_info.get("ipipseek_vpn_checked"):
            quality_sources.append("ipipseek")
...
def check_active_exit_ip_quality(node_id: str) -> tuple[bool, str, dict[str, Any]]:
    ...
    state_updates = {
        ...
        "proxy_scamalytics_checked": False,
        "proxy_scamalytics_error": "",
        "proxy_scamalytics_score": 0,
        "proxy_scamalytics_risk": "",
        "proxy_scamalytics_url": "",
        "proxy_scamalytics_isp": "",
        "proxy_scamalytics_org": "",
        ...
        "proxy_proxycheck_proxy": "",
        "proxy_proxycheck_is_proxy": False,
        "proxy_proxycheck_type": "",
        "proxy_proxycheck_ip": "",
    }
...
def connect_node_with_quality_check(node_id: str) -> str:
    ...
    set_state(
        ...
        active_openvpn_node_id=node_id,
        connected_at=None,
        last_check_message="正在检测出口 IP 质量与 7928 代理下载速度...",
    )
    ...
    try:
        ...
        set_state(
            proxy_quality_ok=False,
            proxy_quality_error=reason,
            is_connecting=False,
            connection_stage="idle",
            quality_checking_node_id="",
            connected_at=None,
...
function render(){
  ...
  if (state.is_connecting || isQualityChecking) {
        ...
        <div class="active-card-info">
          ...
          <div class="active-card-details">
            <div class="active-card-title" style="color: var(--text-primary);">
              <span class="badge" style="background: rgba(245, 158, 11, 0.15); color: #f59e0b; border-color: rgba(245, 158, 11, 0.3);"><span class="badge-pulse" style="background: #f59e0b;"></span>${isQualityChecking ? '质量检测中' : '正在连接'}</span>
              <strong>${esc(isQualityChecking ? '正在验证出口质量，暂不算已连接' : (state.active_node_latency || '正在连接...'))}</strong>
            </div>
            <div class="active-card-meta" style="margin-top: 4px;">
              ${pendingNode ? `${esc(pendingNode.ip || pendingNode.remote_host || pendingNode.id)}:${esc(pendingNode.remote_port || '')} ｜ ` : ''}${esc(state.last_check_message || '正在与 VPN 节点建立加密隧道，请稍候...')}
            </div>
          </div>
        </div>
      </div>
  ...
  : (isQualityChecking && pendingNode
    ? `<span class="badge" style="margin-left:8px; padding:2px 8px; background:rgba(245,158,11,.15); color:#f59e0b; border-color:rgba(245,158,11,.3);">质检中 (${esc(pendingNode.id)})</span>`
    : `<span class="badge unavailable" style="margin-left:8px; padding:2px 8px;">无</span>`);
...
if (checkBtn) {
  ...
  checkBtn.textContent = checkBusy ? "检测中..." : "立即检测补齐";
  checkBtn.style.opacity = checkBusy ? "0.55" : "";
  checkBtn.style.cursor = checkBusy ? "not-allowed" : "";
}
...
$("check").onclick=async()=>{ 
  state.is_maintaining = true;
  state.maintenance_message = "正在启动检测补齐流程...";
  render();
